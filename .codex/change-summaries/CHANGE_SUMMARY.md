...
- 반영 내용:
  ...
  - 닉네임은 3~30자의 영문 소문자·숫자·완성형 한글·단독 자모를 허용합니다. 비밀번호는 6~100자의 영문 대소문자·숫자·`!@#$%^*-=+?/`만 허용합니다.
  - 가입 시 이메일을 사전 확인하고 기존 UNIQUE 제약으로 닉네임 경쟁을 차단합니다. `409`의 `code`로 닉네임·이메일 중복을 구분합니다.
  - 저장 충돌 시 실패한 사용자 추적을 해제하고 인증 상태를 다시 읽습니다. 사용자 생성과 인증 소비의 단일 저장·롤백, 인증 만료 정책과 기존 계정 로그인은 유지합니다.
  - 일회성 앱·MySQL에서 동시 가입과 인증 재사용, 강제 DB 오류 롤백, 요청 제한을 검증하는 PowerShell 스크립트를 추가했습니다. 운영 컨테이너·볼륨은 사용하지 않습니다.
...
## 2026-09-21 - PR 회원가입 중복 닉네임 조기 거절

- 목적: 이미 사용 중인 닉네임에도 비밀번호 해시 계산과 DB 저장 실패·롤백을 수행하던 PR의 성능 퇴행을 작은 변경으로 개선했습니다.
- 변경 영역: `Auth/DbAuthService.cs`, `Auth/RegistrationTests.cs`, 변경 요약.
- 반영 내용:
  - 이메일 중복과 인증 상태를 검증한 뒤, 기존 닉네임 조회 메서드를 재사용해 해시 계산 전에 중복 요청을 거절합니다. 서버 구현 변경은 조건문 5줄입니다.
  - 동시 가입을 처리하는 기존 UNIQUE 충돌 처리와 인증 재사용 동작을 유지합니다. 정상 가입에는 닉네임 조회 1회가 추가되며, 이미 중복인 요청은 해시 계산·저장·롤백을 건너뜁니다.
  - 회귀 테스트 5개로 중복 요청의 저장·인증 소비 방지, 새 닉네임 재시도, 이메일 중복 및 인증 오류의 응답 우선순위를 확인합니다.
